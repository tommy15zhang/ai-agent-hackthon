...
def encode_resized_image(path, size=(128, 128)):
    try:
        ...
        if mime_type:
            if "text" in mime_type:  # Plain text files
                with open(path, "r", encoding="utf-8", errors="ignore") as f:
                    return f.read()
            elif "msword" in mime_type or path.endswith(".docx"):  # Word documents
                doc = Document(path)
                ...
                with open(path, "rb") as f:
                    ...
                    return text if text.strip() else None  # If empty, return None
            elif "vnd.ms-powerpoint" in mime_type or path.endswith(".pptx"):  # PowerPoint files
...
def analyze_image(path):
    try:
        ...
        with open(path, "rb") as image_file:
            base64_image = encode_resized_image(path, size=(128, 128))
...
def AI_Response(path):
    ...
    for file in files:
        ...
        if os.path.isdir(file_path):
            ...
            description = f"This is a file with name {file} and description {analyze_image(file_path)}, created on {datetime.fromtimestamp(os.stat(file_path).st_ctime)}"  # Process images
            file_data.append(f"{file}: {description}")
        else:
            text_content = extract_text(file_path)  # Process text-based files
            if text_content:
                description = f"This is a file with name {file} and first 500 chars: {text_content[:500]}. This file was created on {datetime.fromtimestamp(os.stat(file_path).st_ctime)}"
                file_data.append(f"{file}:\n{text_content[:500]}")  # Limit content to 500 chars
            else:
                file_type = mime_type if mime_type else "unknown"
                description = f"This is a file with name {file}, and file type {file_type}, created on {datetime.fromtimestamp(os.stat(file_path).st_ctime)}"
                file_data.append(f"{file}: {description}")
    ...
    completion = OpenAI().chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an assistant that helps organize files into themed folders based on their names, metadata and content."},
            {"role": "user", "content": "Generate a list of shell commands to create folders and move the files into appropriate themes based on their content. Create a Miscellaneous Folder if approperiate. All files need to be moved into folders."},
    ...
    # Output classification commands

    with open('history.txt', "w") as history:
        history.write(f"prompt token used:{completion.usage.prompt_tokens}, output token used:{completion.usage.completion_tokens}\n")
